...
class _CudnnRnn(Configurable):
    ...
    def __init__(self,
                 ...
                 w_init=tf.keras.initializers.TruncatedNormal(stddev=0.05),
                 recurrent_init=None,
                 bidirectional=True,
    ...
    def _apply_transposed(self, is_train, x):
        ...
        def init(shape, dtype=None, partition_info=None):
            ...
            init_biases = [
                tf.constant(self.lstm_bias / 2.0, tf.float32, (self.n_units,)) if z else tf.zeros(self.n_units)
        ...
        if self.keep_recurrent < 1:
            ...
            recurrent_mask = 1 - recurrent_mask * (
                1 - self.keep_recurrent)  # ones are non-recurrent param, keep_prob elsewhere
...
def _compute_gates(input, hidden, num_units, forget_bias, kernel_init, recurrent_init,
    ...
    def _init_stacked_bias(shape, dtype=None, partition_info=None):
        ...
        shape = list(shape)
        shape[0] = num_units
        inits = [tf.zeros_initializer(), tf.zeros_initializer(), tf.constant_initializer(forget_bias),
...
class BiRecurrentEncoder(SequenceEncoder):
    ...
    def apply(self, is_train, x, mask=None):
        states = \
            bidirectional_dynamic_rnn(self.cell_spec(is_train), self.cell_spec(is_train), x, mask, dtype=tf.float32)[1]
        output = []
        for state in states:
            for i, x in enumerate(state._fields):
                if x == self.output:
