...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="~/model/document-qa/squad")
    ...
    out = args.output_dir + "_" + args.exp_id + "_lr" + str(args.lr) + "-" + datetime.now().strftime("%m%d-%H%M%S")
    dim = args.dim
    ...
    recurrent_layer = CudnnGru(dim, w_init=tf.keras.initializers.TruncatedNormal(stddev=0.05))
    params = trainer.TrainParams(trainer.SerializableOptimizer("Adadelta", dict(learning_rate=args.lr)),
                                 ema=0.999, max_checkpoints_to_keep=2, async_encoding=10,
                                 num_epochs=args.epoch, log_period=30, eval_period=1200, save_period=1200,
    ...
    data = DocumentQaTrainingData(SquadCorpus(corpus_dir), None, batcher, batcher)
